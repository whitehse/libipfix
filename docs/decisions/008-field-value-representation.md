...
- Each field is an `ipfix_field_t` with a `kind` discriminant (`UINT`, `IPV4`, `IPV6`, `MAC`, `STRING`, `RAW`)
- Common IEs are also mirrored into convenience members on `ipfix_data_record_t`
